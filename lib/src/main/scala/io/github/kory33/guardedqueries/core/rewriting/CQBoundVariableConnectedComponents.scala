 ...
 *
 * For example, given a conjunctive query {@code ∃x,y,z. U(c) ∧ U(w) ∧ U(x) ∧ R(x,w) ∧ T(z,c,w)
 * ∧ R(y,z)} with a free variable {@code w} and a constant {@code c},
 *
 *   - the set of bound-variable-free atoms is { U(c), U(w) }, and
 *   - the set of maximally (bound-variable-) connected subqueries is {@code { ∃x. U(x) ∧
 *     R(x,w), ∃y,z. T(z,c,w) ∧ R(y,z) }}.
 *
 * Note that the atom {@code R(x,w)} and the atom {@code T(z,c,w)} are not
 * bound-variable-connected (therefore separated into different subqueries) because the variable
 * {@code w} is free in the CQ.
 *
