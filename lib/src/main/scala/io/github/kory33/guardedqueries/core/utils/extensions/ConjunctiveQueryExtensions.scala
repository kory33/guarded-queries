...
object ConjunctiveQueryExtensions {
  // We enclose the extension methods in the
  // https://github.com/lampepfl/dotty/issues/14777#issuecomment-1078995983
  given Extensions: {} with
    extension (conjunctiveQuery: ConjunctiveQuery)
       ...
       * satisfying a specified predicate. <p> Since {@code ConjunctiveQuery} cannot be empty,
       * an empty {@code Optional} is returned if the predicate is not satisfied by any atom in
       * the given {@code ConjunctiveQuery}.
       */
       ...
       * a specified set of variables. <p> The set of variables in the returned subquery is a
       * subset of {@code variables}, and a variable is bound in the returned subquery if and
       * only if it is bound in {@code conjunctiveQuery}. <p> For example, if {@code
       * conjunctiveQuery} is {@code ∃x,y,z. T(x,y,z) ∧ T(x,y,w) ∧ T(x,c,z)} and {@code
       * boundVariableSet} is {@code {x,y}}, then the returned subquery is {@code ∃x,y.
       * T(x,y,w)}. <p> If no atom in the given {@code ConjunctiveQuery} has variable set
       * entirely contained in {@code variables}, an empty {@code Optional} is returned.
       *
       ...
       * have at least one bound variable in a specified set of variables. <p> For example, if
       * {@code conjunctiveQuery} is {@code ∃x,y,z. T(x,y,w) ∧ T(x,c,z)} and {@code
       * boundVariableSet} is {@code {y}}, then the returned subquery is {@code ∃x,y. T(x,y,w)}.
       * <p> If no atom in the given {@code ConjunctiveQuery} has variable set intersecting with
       ...
       * checks if {@code v} is connected in {@code q}. <p> A set of variables {@code V} is said
       * to be connected in {@code q} if there is at most one {@code q}-connected component of
       * {@code V} in {@code q}.
       */
