...
object ConjunctiveQueryExtensions {
  ...
  extension (conjunctiveQuery: ConjunctiveQuery)
     ...
     * contains at least one bound variable and all bound variables in the atom are present in a
     * specified set of variables. <p> The set of variables in the returned subquery is a subset
     * of {@code variables}, and a variable is bound in the returned subquery if and only if it
     * is bound in {@code conjunctiveQuery}. <p> For example, if {@code conjunctiveQuery} is
     * {@code ∃x,y,z. T(x,y,z) ∧ T(x,y,w) ∧ T(x,c,z)} and {@code boundVariableSet} is {@code
     * {x,y}}, then the returned subquery is {@code ∃x,y. T(x,y,w)}. <p> If no atom in the given
     * {@code ConjunctiveQuery} has variable set entirely contained in {@code variables}, an
    ...
    /**
     * Computes a subquery of a given Conjunctive Query that includes only the atoms which have
     * at least one bound variable in a specified set of variables. <p> For example, if {@code
