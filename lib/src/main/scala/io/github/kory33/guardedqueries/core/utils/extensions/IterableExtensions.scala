...
object IterableExtensions {
  given Extension: AnyRef with
    extension [T](iterable: Iterable[T])
       ...
       */
      def productAll[R](
        mapperToIterable: T => /* immutable */ Iterable[R]
      ): /* immutable */ View[Vector[R]] = {
        val iterablesToProduct = iterable.toVector.map(mapperToIterable(_))
        ...
        new AbstractView[Vector[R]] {
          override def iterator: Iterator[Vector[R]] = new Iterator[Vector[R]] {
            ...
            private val states: Array[ComponentState] =
              Array.tabulate(iterablesToProduct.size)(ComponentState.freshStateAt)
