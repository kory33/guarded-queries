...
import io.github.kory33.guardedqueries.testutils.scalacheck.{GenFormalInstance, GenFormula, GenSet}
...
class FilterNestedLoopJoinSpec extends AnyFlatSpec with ScalaCheckPropertyChecks {
  ...
  def smallNonExistentialQueryOver(predicateSet: Set[Predicate]): Gen[ConjunctiveQuery] = for {
    ...
    queryAtoms <- Gen.listOfN(querySize, Gen.oneOf(predicateSet).flatMap { predicate =>
  ...
  "All homomorphisms in FilterNestedLoopJoin.join" should "materialize a subset of the input instance" in {
    forAll(genInstanceAndJoinQuery, minSuccessful(100)) { case (instance, query) =>
      val joinAlgorithm = new FilterNestedLoopJoin(c => c)
      joinAlgorithm
        .join(query, instance)
        .allHomomorphisms.asScala
        .foreach { homomorphism =>
          val materializedInstance = homomorphism.materializeFunctionFreeAtoms(query.getAtoms().toList.asJava, c => c)
  ...
  val genQueryAndHomomorphism: Gen[(ConjunctiveQuery, HomomorphicMapping[Constant])] = for {
    ...
    homomorphism <- Gen.listOfN(variablesInQuery.size, GenFormula.genConstant(15)).map { constants =>
  ...
  def equivalentAsHomomorphisms(h1: HomomorphicMapping[Constant], h2: HomomorphicMapping[Constant]): Boolean = {
  ...
  "FilterNestedLoopJoin.join" should "find every valid answer" in {
    forAll(genQueryAndHomomorphism, minSuccessful(3000)) { case (query, originalHomomorphism) =>
