...
The class of Guarded Tuple-Generating Dependencies (GTGDs) is an expressive class of first-order formulae for which conjunctive query answering is decidable. Even though the problem is \textsc{2Exptime}-complete in general, it has been shown that, by essentially performing enough pre-computation, one can \emph{rewrite} GTGD conjunctive query answering problem to a Datalog query, which can be run in time polynomial to the size of the input database. Nevertheless, no work has implemented the rewriting algorithm for general conjunctive queries under GTGDs. By utilising the recent implementation for rewriting GTGDs over atomic queries, we revisit the theory of chases, derive a rewriting algorithm for general queries and discuss its implementation and further optimisations.
...
\emph{Query answering} under data integration rules is one of the central problems in knowledge representation and reasoning. To give an idea of the problem, consider the following \emph{relational database}, a collection of tuples in relations.
...
\begin{itemize}
  \item for any $X$, if $U(X)$ is a relation then there should be some value $Z$ with $R(X, Z)$, and
  \item for any $X$, $Y$, $R(X, Y)$ should imply $S(X, X)$.
\end{itemize}

The database $\mathcal{D}$ is \emph{incomplete}, as it does not satisfy the abovementioned constraints. Nevertheless, we could still ask a question of the form: ``\emph{If} the database had been fixed by adding some facts not yet recorded by the information system at the moment, what values $v$ would satisfy $S(v, v)$?'' In the example above, we can say that all of $a$, $b$, and $d$ become answers to this question regardless of how we fix $\mathcal{D}$.
...
More recently, \cite{benedikt_buron_germano_kappelmann_motik_2022} implemented the system for answering \emph{atomic} queries over GTGDs. Their algorithm takes the GTGD rule set $\Sigma$ as an input and outputs a Datalog program that derives all tuples which could have been derived using $\Sigma$. We call this program an \emph{atomic rewriting} of $\Sigma$ to distinguish from rewritings of general conjunctive queries.

To our knowledge, however, no work has yet implemented a query-answering procedure for general conjunctive queries under GTGDs. This project aims to bring the theory closer to implementations using atomic rewritings produced by \cite{benedikt_buron_germano_kappelmann_motik_2022}. Ultimately, we provide a working prototype of a query-answering system that works with GTGDs.
...
In \Cref{chapter:preliminaries}, we review basic terminologies and results that we use throughout the report.

In \Cref{chapter:characterising-query-entailment-under-gtgds}, we revisit \emph{tree-like chase proofs} and introduce the notion of \emph{shortcutting chase trees}. We then analyse how answers to a conjunctive query are embedded into the shortcutting chase tree, emphasising its relationship to the connectedness of the query. At the end of the chapter, we apply our observations to derive a query-answering procedure that operates over shortcutting chase trees.

In \Cref{chapter:deriving-a-rewriting}, we use the observations from \Cref{chapter:characterising-query-entailment-under-gtgds} to plan towards a Datalog rewriting procedure. We will see that the query answering procedure from \Cref{chapter:characterising-query-entailment-under-gtgds} can be used to derive a rewriting, although it is inefficient as is. We close the chapter with optimisations of the algorithm so obtained.

We briefly review our implementation in \Cref{chapter:implementation-and-testing}. Finally, we conclude the report with an overview and discuss some weaknesses of the algorithm to be improved in future studies.
...
\Procedure{DatalogSaturate}{Datalog program $P$, input database $\mathcal{D}$}
  ...
  \While{true}
    ...
    \For{\textbf{each} $r \in P$}
      \For{\textbf{each} $\sigma \in $ result of matching the body of $r$ on $\mathcal{D}_\mathrm{current}$}
...
\label{example:tree-like-chase-proof-example}
  ...
  \end{array}\]
  If we start with an instance $\mathcal{D} = \{A(c_4, c_1)\}$, we can perform a chase step with $\tau_1$, then chase with $\tau_2$, propagate a fact $C(c_4, c_1)$ to the root and finally fire $\tau_3$. We illustrate the process in \Cref{figure:tree-like-chase-proof-example}.
  \qed
\end{example}
...
\begin{figure}[t]
  ...
  \caption{A tree-like chase proof with rules in \Cref{example:tree-like-chase-proof-example}. At each stage, elements newly added to the chase tree are highlighted in red.}
...
\label{proposition:chase-proof-completeness}
  ...
  \begin{proof}[Proof (sketch)] $ $\par
    ...
    ($\Longleftarrow$, "completeness" of chase proofs): This direction is proven in \cite[Proposition 2.6.9]{kappelmann_2019}. The proof therein decomposes an ordinary chase sequence into a tree-like structure, and the decomposition is performed exactly as in \Cref{definition:tree-like-chase-proof}.
  \end{proof}
\end{proposition}
...
\begin{example}
  Consider the rules in \Cref{example:tree-like-chase-proof-example}. According to an implementation in \cite{Benedikt_Rewriting_the_Infinite}, the set $\Sigma'$ of the following two rules is an atomic rewriting of $\Sigma$.
  \[\begin{array}{ccrcl}
    \tau'_1 & = & \forall x_1, x_2.\ B(x_1, x_2) & \rightarrow & C(x_1, c_1) \wedge A(x_2, c_1) \\
    \tau'_2 & = & \forall x_1, x_2.\ A(x_1, x_2) & \rightarrow & C(x_1, c_1)
  \end{array}\]

  A first few layers of $\SCTree{\{ A(c_4, c_1) \}}{\Sigma}$ is illustrated in \Cref{figure:sctree-simple-example}. The chase tree is continued indefinitely by chasing with existential rules and Datalog-saturating each layer with $\Sigma'$.
  \qed
\end{example}
...
\begin{figure}[t]
  ...
  \caption{A shortcutting chase tree over an instance $\{ A(c_4, c_1) \}$ with rules in \Cref{example:tree-like-chase-proof-example}. The facts highlighted in red are obtained by Datalog-saturating the inherited bag with an atomic rewriting, and dotted boxes indicate substituted bodies used to fire an existential rule. Notice how the subtree rooted at $v_1$ can be obtained as a shortcutting chase of the bag at $v_1$, as remarked in \Cref{remark:corecursiveness-of-sctree}.}
...
\label{proposition:each-bag-in-sctree-is-k-sigma-small}
  ...
  \begin{proof}
    By the same analysis as in the proof of \Cref{proposition:chase-proofs-tree-width}.
  \end{proof}
\end{proposition}
...
\label{lemma:sctree-subtree-embeds-into-a-chase-proof}
  ...
  \begin{proof}
    ...
    The base case is $T = \{r\}$, where $r$ is the root of $\SCTree{\mathcal{D}}{\Sigma}$. By \Cref{proposition:chase-proof-completeness}, we can construct a chase-proof $C_1, \ldots, C_n$ that aggregates all provable base facts to the root bag.
    ...
    We extend the proof $C_1, \ldots, C_n$ by applying a chase step with $(\tau, \sigma)$ to create a child node $c \in C_{n+1}$. By \Cref{proposition:chase-proof-completeness}, we can derive all facts whose terms appear in the bag $c$. We have now obtained a chase proof $C_1, \ldots, C_{n+m}$ with $c$ saturated, so $(T, \mu)$ can be homomorphically mapped into $C_{n+m}$.
  \end{proof}
\end{lemma}
...
\label{theorem:sctree-soundness-completeness}
  ...
  \begin{proof}[Proof] $ $\par
    ($\Longrightarrow$): Suppose $\mathcal{D} \cup \Sigma \models Q$. By \Cref{proposition:chase-proof-completeness}, there exists a tree-like chase proof $(T_1, \mu_1), \ldots, (T_n, \mu_n)$ of $Q$ from $\mathcal{D}$ over $\Sigma$. By induction on $1 \leq i \leq n$ and by the construction of $\SCTree{\mathcal{D}}{\Sigma}$, we can embed each $(T_i, \mu_i)$ into $\SCTree{\mathcal{D}}{\Sigma}$. Since $(T_n, \mu_n) \models Q$ and $(T_n, \mu_n)$ embeds into $\SCTree{\mathcal{D}}{\Sigma}$, $\SCTree{\mathcal{D}}{\Sigma} \models Q$.
    ...
    By applying \Cref{lemma:sctree-subtree-embeds-into-a-chase-proof} to $(T, \mu)$, there is a chase proof of $Q$ from $\mathcal{D}$ under $\Sigma$. By soundness of chase proofs (\Cref{proposition:chase-proof-completeness}, $\Longrightarrow$) we are done.
  \end{proof}
\end{theorem}
...
We now discuss how the structure of the query constrains the structure of query homomorphisms into the shortcutting chase tree. By the end of this chapter, we will have derived a recursive query answering procedure, whose recursive structure will be exploited in \Cref{chapter:deriving-a-rewriting} to compute a Datalog rewriting.
...
\begin{figure}[ht]
  ...
  \caption{The hypergraph corresponding to $Q$ in \Cref{example:query-connectedness}.}
\end{figure}
...
\label{lemma:image-of-connected-variables-is-connected}
  ...
  \begin{proof}
    ...
    By \Cref{proposition:term-subtree-is-connected-for-non-rule-constants}, each $V_\sigma(x_i)$ is connected, so there is a path in $\SCTree{\mathcal{D}}{\Sigma}$ that joins $v_1$ and $v_2$ through intersections $V_\sigma(x_i) \cap V_\sigma(x_{i+1})$.
  \end{proof}
\end{lemma}

We will use \Cref{lemma:image-of-connected-variables-is-connected} to develop a query-answering procedure. To begin with, we define a way to split the query entailment checking problem into smaller problems by a partial guess of the query homomorphism.
...
\begin{figure}[ht]
  ...
  \caption{Decomposition of the query from \Cref{example:query-connectedness} with partial map as in \Cref{example:bcq-decomposition}.}
...
\label{example:bcq-decomposition}
  ...
  \begin{align*}
    ...
    &\wedge R(z_3, z_4) \wedge R(z_3, z_5) \wedge T(z_2, z_6, z_5)
  \end{align*}
  as in \Cref{example:query-connectedness}. Suppose that $\sigma_{\mathrm{commit}}$ is given by \[\begin{array}{cccc}
  ...
  \end{align*}
  The decomposition after applying $\sigma_{\mathrm{commit}}$ is illustrated in \Cref{figure:bcq-decomposition-example}.
  \qed
\end{example}
...
We can easily extend \Cref{lemma:bcq-base-connected-query-decomposition} to non-Boolean conjunctive queries.
...
\label{corollary:base-connected-query-decomposition}
  ...
  Then for a substitution $\alpha: \FV(Q) \rightarrow \Terms(\mathcal{D})$, $\mathcal{D} \cup \Sigma \models \alpha(Q)$ if and only if there exists an extension $\sigma_{\mathrm{partial}}: \Vars(Q) \rightharpoonup \Terms(\mathcal{D})$ of $\alpha$ satisfying conditions as in \Cref{lemma:bcq-base-connected-query-decomposition}.
\end{corollary}

In order to produce answers to the query using \Cref{corollary:base-connected-query-decomposition}, we need to decide whether or not $\mathcal{D} \cup \Sigma \models \Subq{Q}{\sigma_{\mathrm{partial}}}{C}$ holds. It turns out that we can exploit the connectedness of $\Subq{Q}{\sigma_{\mathrm{partial}}}{C}$ and the corecursive structure of $\SCTree{\mathcal{D}}{\Sigma}$ (\Cref{remark:corecursiveness-of-sctree}) to check the query entailment recursively. To make this precise, we define the notion of a \emph{successful commit point}, a point at which a connected Boolean conjunctive query can be split in a way similar to \Cref{lemma:bcq-base-connected-query-decomposition}.
...
\label{proposition:bcq-invariant-under-sigma-preserving-renaming}
  ...
  \begin{proof}
    ...
    \begin{cases}
      ...
      t & \text{otherwise}
    \end{cases}$$
    Then $\SCTree{\sigma'(\mathcal{D})}{\Sigma}$ is an isomorphic image of $\SCTree{\mathcal{D}}{\Sigma}$ under $\sigma'$. Now apply \Cref{theorem:sctree-soundness-completeness}.
  \end{proof}
\end{proposition}

\begin{remark}
  In \Cref{proposition:bcq-invariant-under-sigma-preserving-renaming}, we require that the renaming $\sigma$ \emph{preserves} all constants in $\Sigma$. This is because \begin{itemize}
...
\label{proposition:renaming-equivalence-class-bound}
  ...
  \begin{proof}
    For any non-root node $v$ of $\SCTree{\mathcal{D}}{\Sigma}$, by \Cref{proposition:each-bag-in-sctree-is-k-sigma-small} there are at most $K + |\Consts(\Sigma)|$ terms present in the bag $\mu(v)$. Hence there are at most $|P| \cdot 2^{(K + |\Consts(\Sigma)|)}$ facts in $\mu(v)$, where $P$ is the set of predicates in $\mathcal{D} \cup \Sigma$. In particular, the number of $\cong_\Sigma$-equivalence classes in $\SCTree{\mathcal{D}}{\Sigma}$ is at most $2^{|P| \cdot 2^{(K + |\Consts(\Sigma)|)}} + 1$, where the bag at the root node accounts for $(+1)$.
  \end{proof}
\end{proposition}

We are ready to present a query-answering procedure \Cref{algorithm:basic-query-answering} based on the intuition of \Cref{remark:towards-query-answering-procedure}. Note that, in $\textsc{EntailsConnectedBCQ}$, we can scan through all equivalence classes $[\mathcal{D}]_{\cong_\Sigma}$ of bags in $\SCTree{\mathcal{D}}{\Sigma}$ by a depth-first search, since
\begin{itemize}
  \item there are only finitely many such classes by \Cref{proposition:renaming-equivalence-class-bound}, and
...
We will only use \Cref{algorithm:basic-query-answering} as an intermediate step towards producing a rewriting. We will not go through the algorithm line by line, but we shall sketch the proof of its correctness and termination.

\begin{theorem}
  The procedure \textsc{AnswerConjunctiveQuery} in \Cref{algorithm:basic-query-answering} produces all valid answers to \hyperref[GTGDs-CQ-Answering]{GTGDs-CQ Answering} in finite time.
  \begin{proof}[Proof (sketch)]
    ...
    We can first prove the correctness of \textsc{SatisfiedWith} for all connected BCQs by induction on the number of bound variables and recursively applying \Cref{theorem:recursive-connected-bcq-entailment}. It is then straightforward to see, by applying \Cref{corollary:base-connected-query-decomposition}, that \textsc{AnswerConjunctiveQuery} produces all valid answers.
  \end{proof}
\end{theorem}
...
\label{algorithm:basic-query-answering}
\begin{algorithmic}[1]
...
\Procedure{EntailsConnectedBCQ}{$\mathcal{D}$, $\Sigma$, connected BCQ $Q$}
  \For{\textbf{each} equivalence class $[\mathcal{G}]_{\cong_\Sigma}$ of bags in $\SCTree{\mathcal{D}}{\Sigma}$}
    \If{\textsc{IsSuccessfulCommitPoint}($\mathcal{G}$, $\Sigma$, $Q$)}
...
\EndProcedure
\State
\Procedure{IsSuccessfulCommitPoint}{$\mathcal{D}$, $\Sigma$, connected BCQ $Q$}
  \For{\textbf{each} nonempty $\sigma_{\mathrm{commit}}: \Vars(Q) \rightharpoonup \Terms(\mathcal{D}) \setminus \Consts(\Sigma)$}
...
\Procedure{AnswerConjunctiveQuery}{$\mathcal{D}$, $\Sigma$, conjunctive query $Q$}
  \For{\textbf{each} substitution $\alpha: \FV(Q) \rightarrow \Terms(\mathcal{D}) \cup \Consts(\Sigma)$}
    \For{\textbf{each} substitution $\sigma_{\Consts(\Sigma)}: (\Vars(Q) \setminus \FV(Q)) \rightharpoonup \Consts(\Sigma)$}
...
In \Cref{chapter:characterising-query-entailment-under-gtgds}, we developed \Cref{algorithm:basic-query-answering} for producing all answers to a conjunctive query over single-headed GTGDs.

However, because of high data complexity, the algorithm is impractical for query-answering purposes: We have to explore a part of $\SCTree{\mathcal{D}}{\Sigma}$ for every single substitution $\alpha: \FV(Q) \rightarrow \Consts(\mathcal{D})$. So instead, we aim to use \Cref{algorithm:basic-query-answering} as a stepping stone to devising a Datalog rewriting that works for arbitrary input $\mathcal{D}$.
...
\begin{itemize}
  ...
  \item For each \emph{local instance} $I$ (to be defined in \Cref{section:subquery-entailment-problem}), which represents a $(K, \Sigma)$-small structure in the base instance, decide if $I$ contains enough facts to entail a subquery induced by $C$. If so, add a rule roughly of the form $I \rightarrow \mathrm{Subgoal}_C$.
  \item Finally, add all rules that \emph{integrate} subgoals into the goal atom. These rules essentially perform the inverse of \Cref{corollary:base-connected-query-decomposition} by gathering base facts and subgoal atoms together to infer the existence of a query homomorphism.
\end{itemize}

In \Cref{section:subquery-entailment-problem}, we define the precise structure of \emph{local instances} and what it means for one to entail a subquery. We then briefly describe how to enumerate instances that entail a subquery in \Cref{section:naive-subquery-entailment-enumeration}, putting pieces together in \Cref{section:a-rewriting-algorithm} to present a rewriting algorithm. Finally, we discuss in \Cref{section:secomputation-optimisations} some optimisations on the naive algorithm presented in \Cref{section:naive-subquery-entailment-enumeration}.
...
Notice that, as shown in \Cref{figure:sctree-simple-example-local-names}, we can translate a $\SCTree{\mathcal{D}}{\Sigma}$ into a tree-like structure that uses local names. By abuse of notation, we write $\SCTree{I}{\Sigma}$ for the tree-like structure obtained this way.

The structure $\SCTree{I}{\Sigma}$ can be constructed in a way similar to how we construct an ordinary shortcutting chase tree, except that when firing an existential rule, we \emph{reuse} a local name that was inactive at the parent node instead of introducing fresh nulls. For example, at $v'_4$ in \Cref{figure:sctree-simple-example-local-names}, we are using a local name $3$, which is inactive at the parent node $v'_2$, in place of the null $n_4$. We no longer include the local name $1$ in $v'_4$ because $v'_4$ inherits no fact containing $1$. We say that such local names are \emph{dropped} by the chase step. If a local name $n$ dropped at $v$ becomes active again in a descendant $v'$ of $v$, then $n$ at $v$ and $v'$ represent different terms.
...
\begin{example}
  ...
  \begin{align*}
    ...
    &\wedge R(z_3, z_4) \wedge R(z_3, z_5) \wedge T(z_2, z_6, z_5)
  \end{align*}
  as in \Cref{example:query-connectedness}. Then $\partial_Q \{z_4\} = \{z_3\}$, $\partial_Q \{z_2\} = \{z_1, z_3, z_5, z_6\}$ and $\partial_Q \{z_5, z_6\} = \{z_2, z_3\}$. We illustrate the first example in \Cref{figure:query-boundary}.
  \qed
\end{example}
...
\begin{figure}[h]
  ...
  \caption{For the query from \Cref{example:query-connectedness}, the $Q$-boundary $\partial_Q \{z_4\}$ of $\{z_4\}$ (red) is $\{z_3\}$ (blue).}
...
Combining notions defined in \Cref{subsection:local-instances} and \Cref{subsection:partially-substituted-subqueries}, we can formally re-define the \Cref{problem:informal-subquery-entailment}.
...
\begin{problem}[Subquery Entailment Problem, formalisation of \Cref{problem:informal-subquery-entailment}]
...
We call an instance $\mathcal{I}$ a \emph{subquery entailment} if it is an \textsc{yes} instance of \Cref{problem:subquery-entailment}

\section{The Naive Subquery Entailment Enumeration}
\label{section:naive-subquery-entailment-enumeration}

We can solve \Cref{problem:subquery-entailment} with only a minor modification to \Cref{algorithm:basic-query-answering}, with the basic idea being that we descend the $\SCTree{I}{\Sigma}$ to search for a successful commit point. The only difference is that when we fire an existential rule, we must not drop local names that appear in the substituted query $Q_\mathrm{subst}$, since otherwise $Q_\mathrm{subst}$ becomes immediately unsatisfiable in the chased subtree. The first version of our algorithm (\Cref{algorithm:naive-subquery-entailment-enumeration}) is, therefore, a verbatim translation of \Cref{algorithm:basic-query-answering} into the context of local instances.

Later in \Cref{section:secomputation-optimisations}, we discuss optimisations of \Cref{algorithm:naive-subquery-entailment-enumeration}. Nevertheless, for now, we move on to the other components of the rewriting algorithm to see how outputs from \Cref{algorithm:naive-subquery-entailment-enumeration} can be assembled into a Datalog rewriting.

%TC:ignore
\begin{algorithm}
\caption{Naive Subquery Entailment Enumeration}
\label{algorithm:naive-subquery-entailment-enumeration}
\begin{algorithmic}[1]
\State // decide if there exists a $\Consts(\Sigma)$-free query homomorphism
\State // $\sigma: Q \rightarrow \SCTree{I}{\Sigma}$ for a BCQ $Q$ with only
\State // $\LNames{I}$ as query constants
\Procedure{EntailsConnectedBCQ}{$I$, $\Sigma$, connected BCQ $Q$}
  \For{\textbf{each} local instance $I'$ in $\SCTree{I}{\Sigma}$ that can be reached\par
  without dropping local names $\LNames{I} \cap \Consts(Q)$}
    \If{\textsc{IsSuccessfulCommitPoint}($I$, $\Sigma$, $Q$)}
      \State \textbf{return true}
    \EndIf
  \EndFor
  \State \textbf{return false}
\EndProcedure
\State
\Procedure{IsSuccessfulCommitPoint}{$I$, $\Sigma$, connected BCQ $Q$}
  \For{\textbf{each} nonempty $\sigma_{\mathrm{commit}}: \Vars(Q) \rightharpoonup \LNames{I}$}
...
\Procedure{EnumerateSubqueryEntailments}{\par
  ...
  \For{\textbf{each} $(\Sigma, Q)$-subquery entailment problem instance $\mathcal{I}$}
    \State $(C, I, \sigma_{\Consts(\Sigma)}, \sigma_I, \iota) \gets \mathcal{I}$
...
For example, suppose that the subquery in \Cref{example:local-realisation-of-subquery-representation} is entailed by the local instance $I$ given in the example. We would then like to add a rule $$
...
\label{proposition:subgoal-captures-subquery-fulfilment}
  ...
  \begin{proof} $ $\par
    ...
    ($\Longrightarrow$):
      Let $\rho$, $I$, and $S$ be as defined in \Cref{definition:subgoal-rule}. Let $\lambda$ be an assignment that maps each variable $x_n$ in $\mathrm{SubgoalRule}(\mathcal{I})$ to a local name $n$. Then by applying the substitution $\lambda$, we have $\Sigma \cup \{ \Phi_{Q, C} \} \models \iota^{-1}(I \rightarrow S)$. Since $S$ does not contain a local name, $$
...
\label{example:glueing-rule}
  ...
  \end{align*}
  $Q$ is similar to the query in \Cref{example:query-connectedness}, except except that we replaced $z_2$ and $z_3$ with free variables $w_2$ and $w_3$. If we let $\mathrm{BVars} = \{w_2, w_3, z_5\}$, then \begin{align*}
...
\begin{algorithmic}[1]
\Procedure{Rewrite}{Single-headed GTGDs $\Sigma$, Conjunctive query $Q$}
  \State $result \gets \emptyset$
  \State $result.addAll(\ARew{\Sigma})$
  \For{\textbf{each} $\mathcal{I} \in $\textsc{EnumerateSubqueryEntailments}($\Sigma$, $Q$)}
...
\begin{theorem}
  Let $\Sigma$ be a finite set of single-headed GTGDs and $Q$ a conjunctive query. Then \textsc{Rewrite}$(\Sigma, Q)$ in \Cref{algorithm:gtgds-cq-rewriting-procedure} computes a Datalog rewriting of $(\Sigma, Q)$.
  \begin{proof}
    By correctness of \Cref{algorithm:naive-subquery-entailment-enumeration}, \Cref{proposition:subgoal-captures-subquery-fulfilment} and \Cref{remark:glueing-rule-soundness-and-completeness}.
  \end{proof}
\end{theorem}
...
When rewriting a GTGDs-CQ pair ($\Sigma, Q$) with \Cref{algorithm:gtgds-cq-rewriting-procedure}, the apparent bottleneck is the call \textsc{EnumerateSubqueryEntailments}($\Sigma, Q$) because we need to
\begin{itemize}
  \item visit every $(\Sigma, Q)$-subquery entailment problem instance $\mathcal{I}$ and test if $\mathcal{I}$ satisfies \Cref{problem:subquery-entailment}, and
...
\label{remark:naive-seenumeration-inefficiencies}
  As an illustrating example, consider the query $$Q = \exists z_1,z_2,z_3.\ S(z_1, z_2) \wedge R(z_1, z_3).$$ We remark on the following inefficiencies with the naive implementation \Cref{algorithm:naive-subquery-entailment-enumeration} of \textsc{EnumerateSubqueryEntailments}:
...
In the remainder of this chapter, we describe three optimisation techniques: two that address the issue (3) in \Cref{subsection:dp-optimisation} and in \Cref{subsection:dfs-optimisation}, and the other that partially resolves (1) in \Cref{subsection:instance-normalisation}. However, we leave (2) as a problem for future work.

\subsection{Dynamic Programming}
\label{subsection:dp-optimisation}

As observed in \Cref{remark:naive-seenumeration-inefficiencies}, the issue is that
\begin{itemize}
...
To proceed, we need to define the process of ``splitting" a subquery entailment problem instance, a process which is unsurprisingly similar to \Cref{definition:query-splitting}.
...
\label{definition:subquery-entailment-problem-splitting}
  ...
  \end{align*}
  and $S_{Q, C'}$ defined as in \Cref{definition:embedding-of-constants-adjacent-to-c}.
...
%TC:ignore
\begin{algorithm}
\caption{Recursive Subquery Entailment Enumeration}
\label{algorithm:recursive-subquery-entailment-enumeration}
\begin{algorithmic}[1]
\State // decides if $\mathcal{I}$ is a $(\Sigma, Q)$-subquery entailment
\State // we can optionally cache the answer of this function using a hash map
\Procedure{IsSubqueryEntailment}{$\mathcal{I} = (C, I, \sigma_{\mathrm{\Consts(\Sigma)}}, \sigma_I, \iota)$}
  \For{\textbf{each} local instance $I'$ in $\SCTree{I}{\Sigma}$ that can be reached\par
  without dropping local names $\range{\sigma_I} \cup \range{\iota}$}
    \If{\textsc{Splits}($(C, I', \sigma_{\mathrm{\Consts(\Sigma)}}, \sigma_I, \iota)$)}
      \State \textbf{return true}
    \EndIf
  \EndFor
  \State \textbf{return false}
\EndProcedure
\State
\State // decides if $\mathcal{I}$ splits into subquery entailments
\Procedure{Splits}{$\mathcal{I} = (C, I, \sigma_{\mathrm{\Consts(\Sigma)}}, \sigma_I, \iota)$}
  \For{\textbf{each} nonempty $\sigma_{\mathrm{commit}}: C \rightharpoonup \LNames{I}$}
    \If{\textsc{SplitsWith}($\mathcal{I}$, $\sigma_{\mathrm{commit}}$)}
      \State \Return{\textbf{true}}
    \EndIf
  \EndFor
  \State \Return{\textbf{false}}
...
\State
\State // decides if $\mathcal{I}$ splits into subquery entailments
\State // with commit map $\sigma_{\mathrm{commit}}: \Vars(Q) \rightharpoonup \LNames{I}$
\Procedure{SplitsWith}{$\mathcal{I} = (C, I, \sigma_{\mathrm{\Consts(\Sigma)}}, \sigma_I, \iota)$, $\sigma_{\mathrm{commit}}$}
  \State $I_\mathrm{Dsat} \gets \textsc{DatalogSaturate}(\ARew{\Sigma}, I)$
  \State $\mathrm{baseSatisfied} \gets I_\mathrm{Dsat} \models \Commq{\mathcal{I}}{\sigma_{\mathrm{commit}}}$
  \State $C_1, \ldots, C_n \gets$ $Q$-connected components of $C \setminus \dom{\sigma_{\mathrm{commit}}}$
  \State $\mathrm{allComponentsSatisfied} \gets \bigwedge_{i = 1}^n$
  \State \hskip1.5em \textsc{IsSubqueryEntailment}($\Subi{\mathcal{I}}{\sigma_{\mathrm{commit}}}{C_i}$)
  \State \Return{baseSatisfied \textbf{and} allComponentsSatisfied}
\EndProcedure
\State
\Procedure{EnumerateSubqueryEntailments}{\par
  finite set $\Sigma$ of single-headed GTGDs, conjunctive query $Q$}
  \For{\textbf{each} $(\Sigma, Q)$-subquery entailment problem instance $\mathcal{I}$}
    \If{\textsc{IsSubqueryEntailment}($\mathcal{I}$)}
      \State \textbf{output} $\mathcal{I}$
    \EndIf
  \EndFor
\EndProcedure
...
\Cref{algorithm:recursive-subquery-entailment-enumeration} is another implementation of \nobreak\textsc{EnumerateSubqueryEntailments} based on \Cref{definition:subquery-entailment-problem-splitting}. The algorithm is equivalent to \Cref{algorithm:naive-subquery-entailment-enumeration}, but since \textsc{IsSubqueryEntailment} is a Boolean function over $(\Sigma, Q)$-subquery entailment problem instances, we can memoise answers of \textsc{IsSubqueryEntailment} and avoid recomputations.

\subsection{DFS Optimisation}
\label{subsection:dfs-optimisation}

As discussed briefly at the end of \Cref{chapter:characterising-query-entailment-under-gtgds}, instead of computing the set of all local instances $I'$ satisfying the condition at line 4 of \Cref{algorithm:recursive-subquery-entailment-enumeration}, we can use a depth-first search (DFS) to explore the space of all such local instances.

During this process of deciding whether $\mathcal{I} = (C, I, \sigma_{\mathrm{\Consts(\Sigma)}}, \sigma_I, \iota)$ is a subquery entailment, each instance $I'$ we encounter in the DFS corresponds to a problem instance $\mathcal{I'} = (C, I', \sigma_{\mathrm{\Consts(\Sigma)}}, \sigma_I, \iota)$. If we use the DP optimisation, we can check if we have already seen $\mathcal{I'}$, and if so, immediately stop exploring the chase tree. We illustrate this process in \Cref{figure:sctree-dfs-process}.

\begin{figure}[t]
\centering
\includesvg{./diagrams/sctree-dfs-process.drawio.svg}
...
If we find a node $v$ that admits a splitting into subquery entailments, we mark all ancestors of $v$ as \textbf{true}, and if the exploration below a node $v$ is unsuccessful, we mark $v$ \textbf{false}, move up and try siblings of $v$. We show this marking process in \Cref{figure:sctree-dfs-marking}.

\begin{figure}[t]
  \centering
  \includesvg{./diagrams/sctree-dfs-marking.drawio.svg}
...
Finally, we address the first point of \Cref{remark:naive-seenumeration-inefficiencies} concerning isomorphic instances.

Recall that we use $2K$ local names as the set of $K$ names is insufficient to maintain a tree-like structure of local instances (\Cref{figure:sctree-simple-example-equivalence-classes}).

However, in \Cref{algorithm:recursive-subquery-entailment-enumeration}, even with DP and DFS optimisations in place, we only partially use the structure between local instances when we descend the chase tree. All we require is that the local names in $\range{\sigma_I} \cup \range{\iota}$ be preserved. In particular, we do not care if a local name (not in $\range{\sigma_I} \cup \range{\iota}$) at a node $v$ is introduced at $v$ or is inherited from the parent of $v$.
...
\label{chapter:implementation-and-testing}

We built an \href{https://github.com/kory33/guarded-queries}{open-sourced} prototypical rewriting library in Java, combining all ideas from \Cref{chapter:characterising-query-entailment-under-gtgds} and \Cref{chapter:deriving-a-rewriting}. On top of the core library, the system has a command-line interface where an end-user can interact with the rewriter.
...
The implementation of the rewriter closely follows \Cref{algorithm:gtgds-cq-rewriting-procedure}. For \\ \textsc{EnumerateSubqueryEntailments}, we provide three implementations based on \Cref{algorithm:recursive-subquery-entailment-enumeration}, which respectively have (\emph{DP}), (\emph{DP + normalisation}) and (\emph{DP + normalisation + DFS}) optimisations applied.
...
We illustrate the overall architecture in \Cref{figure:implementation-architecture}.

\begin{figure}[h]
\centering
\includesvg{./diagrams/rewriter-architecture.drawio.svg}
...
\Cref{figure:example-command-line-interaction} is an example of an interaction between a user and the application where the user attempts to rewrite
$$
...
\end{Verbatim}
%TC:endignore
\caption{An interaction with the command-line interface. The user uses the algorithm with all three optimisations applied.}
\label{figure:example-command-line-interaction}
\end{figure}

For comparison, with only (DP) and (DP + normalisation) optimisations applied, rewriting the same input as in \Cref{figure:example-command-line-interaction} takes about 7.3 seconds and 160 seconds respectively, demonstrating the effectiveness of optimisations in \Cref{section:secomputation-optimisations}.
