...
It is known that a conjunctive query under GTGD rules admits \emph{GTGD-Datalog rewritings}. These are Datalog programs together with a designated \emph{goal atom} that, when run on any base instance, give the same set of answers as the original rule-query pair. More precisely:

\begin{definition}
  A \emph{GTGD-Datalog rewriting} of a GTGD-CQ pair $(\Sigma, Q)$ is a Datalog program $\Sigma_\mathrm{Datalog}$ together with an atomic query $Q_\mathrm{atomic}$ such that for every base instance $\mathcal{D}$ and a substitution $\sigma: FV(Q) \rightarrow \Consts(\mathcal{D})$ for $Q$, $$\Sigma, \mathcal{D} \models \sigma(Q) \textit{ if and only if } \Sigma_\mathrm{Datalog}, \mathcal{D} \models \sigma(Q_\mathrm{atomic}).$$
  We call $Q_\mathrm{atomic}$ the \emph{goal atom} in the GTGD-Datalog rewriting.
\end{definition}

This project aims to derive an algorithm that produces GTGD-Datalog rewritings for arbitrary GTGD-CQ pairs.

\newpage
\chapter{Structures of GTGD Chases}

To begin a journey towards \emph{GTGD-Datalog rewritings} with, consider answering an existential-free query $Q$ over a Datalog program $P$ and an input database $\mathcal{D}$.
...
We call the database instance $\textsc{DatalogSaturate}(P, \mathcal{D})$ the \emph{Datalog saturation of $\mathcal{D}$ with $P$}. Intuitively, a Datalog saturation is the result of \emph{repairing} the input instance by adding all but nothing other than facts derivable from the original instance.

In the settings of (possibly existential) TGDs, objects known as \emph{chases} serve a similar role of representing \emph{repaired} database instances. Chases are constructed similarly to Datalog saturations by adding tuples produced by rules, except we need to replace existential variables with \emph{nulls}, which represent fresh constants. 

% TODO: Review this list
In this chapter, we look at several classes of objects related to chases. Namely, these are chase sequences, chase proofs and shortcutting chase trees. They have overlapping semantics but are helpful in different contexts.

\section{Chase Sequences}

% TODO: Fill in the detail here.

\section{Tree-Like Chase Proofs}

%TODO: Review: if we use chase sequences at the beginning, the following is not the motivation for introducing chase proofs. I think chase proofs need to come here to have a structured finite slice of the chase, but I am not sure if we should put the chase sequence first.
The problem with chases is that they are typically infinite for GTGDs rule sets in general, and one cannot wish to compute the entire chase to answer GTGD-CQ-Answering problems. To deal with this, we now turn to the notion of \emph{tree-like chase proofs}, which essentially captures instants of ongoing chasing processes.

\begin{definition}
  A \emph{chase tree} is a pair of a (potentially infinite) rooted directed tree $T$ together with a function $\mu$ mapping each vertex $v \in T$ to database instances (possibly with nulls).
\end{definition}

Chase trees can be infinite, but we now focus on finite chase trees. The following definition is due to \cite{benedikt_buron_germano_kappelmann_motik_2022}.
...
A tree-like chase proof over $\Sigma$ from $\mathcal{D}$ represents a process of repairing $\mathcal{D}$ according to $\Sigma$. We can derive new tuples by using a chase step, branching off to a child node when introducing nulls corresponding to existential values. With a propagation step, we can retrieve a fact derived at a descendant node to the ancestor node.

Note that we only propagate or inherit facts whose terms appear in the target instance. Intuitively, such a restriction can be justified because the guardedness of rules in $\Sigma$ prohibits us from \emph{combining} unrelated terms into new tuples. We cannot hope to fire an existential rule from a node $a \in T$ in a meaningfully novel manner even if we transfer to $a$ a fact with terms not already present in $a$. That is, if we propagated a fact $F$ from $v$ to $a$ and could fire a rule $(\tau, \sigma)$ on $a$ \emph{using} $F$, we could have fired $(\tau, \sigma)$ on $v$ in the first place, provided that $v$ inherited enough facts related to $F$ from $a$\footnote{In fact, $a$ might have more facts with terms in $F$ than $v$ does due to propagation from cousin nodes of $v$. In that case, we could "re-derive" a cousin node $v'$ from $a$ in the same way as how $v$ had been derived, and $v'$ should have as many facts related to an isomorphic copy of $F$ as $a$ does.}.

The restriction mentioned above makes tree-like chase proofs locally compact in the following sense.

\begin{definition}
  Let $\Sigma$ be a set of GTGDs and $K \in \mathbb{N}$. We say that an instance $\mathcal{D}$ is \emph{$(K, \Sigma)$-small} when $|\Terms(\mathcal{D}) \setminus \Consts(\Sigma)| \leq K$.
\end{definition}

\begin{proposition}[Tree-width of chase proofs]
\label{chase-proofs-tree-width}
  Let $\Sigma$ be a set of \emph{single-headed} GTGDs, $(T_1, \mu_1), \ldots, (T_n, \mu_n)$ a tree-like chase proof over $\Sigma$ from $\mathcal{D}$ and $r \in T_1$ the root node. Let $K$ be the maximum arity of predicates that appear in $\mathcal{D} \cup \Sigma$. Then for every $1 \leq i < n$ and every non-root vertex $v$ of $T_i$, $\mu_i(v)$ is $(K, \Sigma)$-small. In particular, if $\mathcal{D}$ is $(K, \Sigma)$-small, all instances in $(T_i, \mu_i)$ are $(K, \Sigma)$-small.
  \begin{proof}
    By induction on $i$. We look at the rule used to derive $(T_{i+1}, \mu_{i+1})$ from $(T_{i}, \mu_{i})$ and examine the instance at the modified node.
    \begin{itemize}
      \item If the last step was a chase step with a full rule or a propagation step, no term set has been modified for any instance in the chase tree.
      \item If the last step was a chase step from a vertex $v \in T_i$ with an existential rule $\forall \vec{x}. \beta \rightarrow \exists \vec{y}. H$ and a substitution $\sigma$, we have added a new node $c$ under $v$. As $|\Terms(\sigma'(H))| \leq K$ and the inherited instance $I$ has $\Terms(I) \subseteq \Terms(\sigma'(H)) \cup \Consts(\Sigma)$,
      \begin{equation*}
        \begin{split}
...
Tree-like chase proof is a powerful tool to reason about a fragment of a chase, but unlike a full chase, they do not constitute a model of $\mathcal{D} \cup \Sigma$.  Instead, we want a universal model as structured as a chase proof.
...
\end{itemize}
We can immediately achieve the former objective by computing the Datalog saturation of the instance at $n$ using $\ASatRules{\Sigma}$. This observation motivates the following definition.
...
\begin{definition}[Shortcutting Chase Trees]
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item the instance $I_{l, \tau, \sigma}$ \emph{inherited from $\mu_i(l)$ through $\tau$ and $\sigma$} as an instance $$I_{l, \tau, \sigma} = \{ F \in \mu_i(l) \mid \Terms(f) \subseteq \Terms(\sigma'(H)) \cup \Consts(\Sigma) \}$$
    \end{itemize}
    We construct $T_{i+1}$ as an extension of $T_i$ by adding a vertex $c_{l, \tau, \sigma}$ as a child of $l$ for each such $l \in L_i$, $\tau$ and $\sigma$. We extend $\mu_i$ to $\mu_{i+1}$ by setting $$\mu_{i+1}(c_{l, \tau, \sigma}) = \{ \sigma'(H) \} \cup I_{l, \tau, \sigma}.$$
  \end{itemize}
...
\begin{proposition}
  Let $\Sigma$ be a set of single-headed GTGDs and $\mathcal{D}$ a base instance. Let $K$ be the maximum arity of predicates that appear in $\mathcal{D} \cup \Sigma$. Then every instance at a non-root node of \emph{$\SCTree{\mathcal{D}}{\Sigma}$} is $(K, \Sigma)$-small. In particular, if $\mathcal{D}$ is $(K, \Sigma)$-small, all instances in $\SCTree{\mathcal{D}}{\Sigma}$ are $(K, \Sigma)$-small.
  \begin{proof}
    By an analysis as in the proof of \ref{chase-proofs-tree-width}.
  \end{proof}
\end{proposition}

Expectedly, $\SCTree{\mathcal{D}}{\Sigma}$ is in fact a universal model for $\mathcal{D} \cup \Sigma$.
...
\section{Homomorphisms into Shortcutting Chase Trees}
...
\newpage
\chapter{Implementation, Verification and Evaluation}
