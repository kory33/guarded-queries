...
\Procedure{IsSubqueryEntailment}{$\mathcal{I} = (C, I, \sigma_{\mathrm{\Consts(\Sigma)}}, \sigma_I, \iota)$}
  ...
  without dropping local names $\range{\sigma_I} \cup \range{\iota}$}
    \If{\textsc{IsSuccessfulCommitPoint}($(C, I', \sigma_{\mathrm{\Consts(\Sigma)}}, \sigma_I, \iota)$)}
      \State \textbf{return true}
    \EndIf
  \EndFor
  \State \textbf{return false}
...
% TODO: write about DFS

\subsection{Instance Normalisation}
\label{subsection:instance-normalisation}

% TODO: Write about re-normalisation of local instances to {0, ..., K-1}.
