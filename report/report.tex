...
\begin{theorem}
  The procedure \textsc{AnswerConjunctiveQuery} in \Cref{basic-query-answering-procedure} produces all valid answers to \hyperref[GTGDs-CQ-Answering]{GTGDs-CQ Answering} in finite time.
...
In \Cref{subquery-entailment-problem-section}, we define the precise structure of \emph{local instances} and what it means for one to entail a subquery. We then briefly describe how to enumerate instances that entail a subquery in \Cref{section:naive-subquery-entailment-enumeration}, putting pieces together in \Cref{a-rewriting-algorithm} to present a rewriting algorithm. Finally, we discuss in \Cref{secomputation-optimisations} some optimisations on the naive algorithm presented in \Cref{section:naive-subquery-entailment-enumeration}.
...
We call an instance $\mathcal{I}$ a \emph{subquery entailment} if it is an \textsc{yes} instance of \Cref{subquery-entailment-problem}
...
Later in \Cref{secomputation-optimisations}, we discuss optimisations of \Cref{naive-subquery-entailment-enumeration}. Nevertheless, for now, we proceed to the other components of the rewriting algorithm to see how outputs from \Cref{naive-subquery-entailment-enumeration} can be assembled into a Datalog rewriting.
...
\label{example:glueing-rule}
  ...
  \end{align*}
  $Q$ is similar to the query in \Cref{query-connectedness-example}, except except that we replaced $z_2$ and $z_3$ with free variables $w_2$ and $w_3$. If we let $\mathrm{BVars} = \{w_2, w_3, z_5\}$, then \begin{align*}
...
\label{glueing-rule-soundness-and-completeness}
  Each $\mathrm{SglGlueingRule}_\mathrm{BVars}$ is ``sound" (in a sense as in \Cref{subgoal-captures-subquery-fulfilment}, by identifying subgoals with subquery fulfilments and the goal atom with query fulfilment), and also collectively complete (i.e. we can derive all answers to $Q$ as goal facts if we can use all glueing rules) by \Cref{base-connected-query-decomposition}.
\end{remark}

\subsection{Putting Pieces Together}

Finally, we combine components from \Cref{section:naive-subquery-entailment-enumeration}, \Cref{subquery-entailment-to-datalog-rule} and \Cref{glueing-subgoals}.
...
\begin{theorem}
  Let $\Sigma$ be a finite set of single-headed GTGDs and $Q$ a conjunctive query. Then \textsc{Rewrite}$(\Sigma, Q)$ in \Cref{gtgds-cq-rewriting-procedure} computes a Datalog rewriting of $(\Sigma, Q)$.
  \begin{proof}
    By correctness of \Cref{naive-subquery-entailment-enumeration}, \Cref{subgoal-captures-subquery-fulfilment} and \Cref{glueing-rule-soundness-and-completeness}.
  \end{proof}
\end{theorem}
...
When rewriting a GTGDs-CQ pair ($\Sigma, Q$) with \Cref{gtgds-cq-rewriting-procedure}, the apparent bottleneck is the call \textsc{EnumerateSubqueryEntailments}($\Sigma, Q$) because we need to
\begin{itemize}
  \item visit every $(\Sigma, Q)$-subquery entailment problem instance $\mathcal{I}$ and test if $\mathcal{I}$ satisfies \Cref{subquery-entailment-problem}, and
...
\label{naive-seenumeration-inefficiencies}
  As an illustrating example, consider the query $$Q = \exists z_1,z_2,z_3.\ S(z_1, z_2) \wedge R(z_1, z_3).$$ We remark on the following inefficiencies with the naive implementation \Cref{naive-subquery-entailment-enumeration} of \textsc{EnumerateSubqueryEntailments}:
...
In the remainder of this chapter, we describe three optimisation techniques: two that address the issue (3) in \Cref{subsection:dp-optimisation} and in \Cref{subsection:dfs-optimisation}, and the other that partially resolves (1) in \Cref{subsection:instance-normalisation}. However, as we shall briefly discuss in \Cref{section:limitations-and-future-work}, we leave (2) as a problem for future work.

\subsection{Dynamic Programming}
\label{subsection:dp-optimisation}

As observed in \Cref{naive-seenumeration-inefficiencies}, the issue is that
\begin{itemize}
...
To proceed, we need to define the process of ``splitting" a subquery entailment problem instance, a process which is necessarily similar to \Cref{definition:query-splitting}.
...
\Cref{algorithm:recursive-subquery-entailment-enumeration} is another implementation of \nobreak\textsc{EnumerateSubqueryEntailments} based on \Cref{definition:subquery-entailment-problem-splitting}. The algorithm is equivalent to \Cref{naive-subquery-entailment-enumeration}, but since \textsc{IsSubqueryEntailment} is a boolean function of $(\Sigma, Q)$-subquery entailment problem instances, we can memoise answers of \textsc{IsSubqueryEntailment} and avoid recomputations.

\subsection{DFS Optimisation}
\label{subsection:dfs-optimisation}

As discussed briefly at the end of \Cref{characterising-query-entailment-under-gtgds}, instead of computing the set of all local instances $I'$ satisfying the condition at line 4 of \Cref{algorithm:recursive-subquery-entailment-enumeration}, we can use a depth-first search (DFS) to explore the space of all such local instances.
...
We call this efficient traversal of the chase tree the \emph{DFS optimisation}. Combined with DP optimisation from \Cref{subsection:dp-optimisation}, we never need to compute \textsc{splits} twice for the same instance.

\subsection{Instance Normalisation}
\label{subsection:instance-normalisation}

Finally, we address the first point of \Cref{naive-seenumeration-inefficiencies} concerning isomorphic instances.

Recall that we use $2K$ local names since the set of $K$ names is insufficient to maintain a tree-like structure of local instances (\Cref{sctree-simple-example-equivalence-classes}).
...
We built an \href{https://github.com/kory33/guarded-queries}{open-sourced} prototypical rewriting system in Java by combining all ideas from \Cref{characterising-query-entailment-under-gtgds} and \Cref{deriving-a-rewriting}. The system has a command-line interface where an end-user can interact with the rewriter.
...
\caption{The overall architecture of the rewriting system. (Top) The rewriter is a composition of an atomic rewriter with an implementation of \textsc{EnumerateSubqueryEntailments}. Every dotted line indicates a call through an interface. The architecture is flexible in that one can apply optimisations to one or more components independently. (Bottom) when a CLI user or an integration test performs a rewriting, the result is minimised by a subsumption logic.}
...
Finally, we provided an implementation of the algorithm, which is capable of handling arity-2 rules and a few constants in the rule and the query.

\section{Liminations and Future Work}
\label{section:limitations-and-future-work}

The current implementation suffers from blowups, doubly exponential in the maximum arity of the input signature and exponential in the number of constants and predicates (\Cref{remark:rewriting-complexity}). Our system, therefore, cannot be applied as-is to real-world ontologies, which typically contain many constants and predicates.

To deal with a higher number of constants and predicates, we must trim down the subquery entailment problem space as much as possible, possibly by analysing the structure of the input rules. For instance, if a rule constant $c$ only appears in a head, it does not make sense to consider a local instance containing $c$ because there is no additional rule we can fire by having such a fact. Another crucial thing to consider is the possibility of instance subsumption, as discussed in \Cref{naive-seenumeration-inefficiencies}: If a single atom $R(1, 2)$ suffices to entail a subquery, \emph{all} local instances containing any fact with $R$ no longer need to be tested for entailment, drastically reducing the search space.

As remarked in \Cref{section:tests}, we can rewrite some queries into atomic queries by adding a few guarded rules. However, our system does not perform any such preprocessing, nor does it reduce subquery entailment problems to atomic queries, even if induced subqueries are acyclic.
