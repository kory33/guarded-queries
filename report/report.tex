...
\Procedure{EnumerateSubqueryEntailments}{\par
  ...
  \For{\textbf{each} $Q$-subquery entailment instance $\mathcal{I} = (C, I, \sigma_{\Consts(\Sigma)}, \sigma_I, \iota)$}
    \If{\textsc{BCQSatisfiedWith}($I$, $\Sigma$, $\LRealz{Q}{\sigma_{\Consts(\Sigma)}, \sigma_I, \iota}$, $\emptyset$)}
...
\begin{theorem}
  Let $\Sigma$ be a finite set of single-headed GTGDs and $Q$ a conjunctive query. Then \textsc{Rewrite}$(\Sigma, Q)$ in \Cref{gtgd-cq-rewriting-procedure} computes a Datalog rewriting of $(\Sigma, Q)$.
  \begin{proof}
