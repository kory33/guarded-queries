...
\begin{problem}
   Given an existential free conjunctive query $Q$, an input database $\mathcal{D}$ and a Datalog program $P$, find all substitutions $\sigma: FV(Q) \rightarrow \Consts(\mathcal{D})$ such that $\mathcal{D} \cup P \models \sigma(Q)$.
\end{problem}
...
\label{GTGD-CQ-Answering}
  Given a finite set $\Sigma$ of GTGD rules, a conjunctive query $Q = \exists \vec{z}. \bigwedge_{j \in J} A_j(\vec{u})$ and a base instance $\mathcal{D}$, what are the \emph{answers} to $Q$ under $\Sigma$ and $\mathcal{D}$, i.e. substitutions $\sigma: FV(Q) \rightarrow \Consts(\mathcal{D})$ such that $\mathcal{D} \cup \Sigma \models \sigma(Q)$?
\end{problem}
...
For a shortcut chase tree $\SCTree{\mathcal{D}}{\Sigma}$ and a child $c$ of the root of the tree, we call the subtree $\SCTree{\mathcal{D}}{\Sigma} \downarrow c$ a \emph{tentacle} in $\SCTree{\mathcal{D}}{\Sigma}$. We observe that the set of null variables in $Q$ connected by atoms must occur within the same tentacle.
...
\begin{definition}
  ...
  \begin{itemize}
    ...
    \item a set $Z$ of variables bound in $Q$ are \emph{$Q$-connected} if for each pair $z_1, z_2 \in Z$ of variables, there is a finite sequence $v_1, \ldots, v_n$ of variables in $Z$ such that
    \begin{itemize}
      \item $v_1 = z_1$ and $v_n = z_2$
      \item for each $1 \leq i < n$, $v_i$ and $v_{i+1}$ are $Q$-adjacent
    \end{itemize}
  \end{itemize}
  For a subset $Z$ of $\vec{z}$, a \emph{$Q$-connected component of $Z$} is a maximally $Q$-connected subset of $Z$.
  \qed
\end{definition}

\begin{proposition}[Co-existentialness of nulls]
\label{coexistentialness}
  Let $Q = \exists \vec{z}. \bigwedge_{j \in J} A_j$ be a conjunctive query, $\sigma: Q \rightarrow \SCTree{\mathcal{D}}{\Sigma}$ be a homomorphism, and $C$ a $Q$-connected component of $\NullVars{\sigma}$. If we write $T_C$ for the set of nodes in $\SCTree{\mathcal{D}}{\Sigma}$ all of whose bags contain a null in $\sigma(C)$, then $T_C$ is a proper subtree of $\SCTree{\mathcal{D}}{\Sigma}$. In particular, all nulls in $\sigma(C)$ occur in a single tentacle.
  \begin{proof}
    Since $T_C$ does not contain the root node, it suffices to show the connectedness of $T_C$. (TODO).
  \end{proof}
\end{proposition}

In the light of \Cref{coexistentialness}, we call a $Q$-connected component of $\NullVars{\sigma}$ a \emph{set of variables co-existential according to $\sigma$}.

The decomposition of query variables into $\textrm{BaseVars}$ and $\textrm{NullVars}$ allows us to separate the conjunctive query answering problem into the following two steps.

\begin{enumerate}
  \item (Non-deterministically) guess the set $B$ of base variables such that $B \supset FV(Q)$. Guess how a query homomorphism restricts to $B$, i.e. guess a mapping $\sigma_\mathrm{base}: B \rightarrow \Consts(\mathcal{D})$.
  \item Check that the query is satisfied in a way compatible to $\sigma_\mathrm{base}$, in the following \emph{decomposed} manner:
  \begin{itemize}
    \item Let $$J_B = \{ j \in J \mid \Vars(A_j) \subseteq B \}.$$ Check that for each $j \in J_B$, $$\sigma_\mathrm{base}(A_j) \in \textsc{DatalogSaturate}(\ASatRules{\Sigma}, \mathcal{D})$$
    \item For each $Q$-connected component $C$ of $\Vars(Q) \setminus B$, let $Q_C$ be the boolean conjunctive query given by $\exists (\vec{z} \setminus B). \bigwedge_{j \in J_{C \cup B}} \sigma_\mathrm{base}(A_j)$, where $$J_{C \cup B} = \{ j \in J \mid \Vars(A_j) \subseteq C \cup B \}.$$Check that $\mathcal{D} \cup \Sigma \models Q_C$ holds for each $C$.
  \end{itemize}
\end{enumerate}

The crucial observation, which we will exploit in the next chapter, is that the satisfaction of each $Q_C$ only depends on the set of facts that is propagated down to the tentacle.

% TODO: state (since the proof is trivial) that the subquery entailment only depends on the set of facts propagated to the root of the tentacle in which the query is witnessed.
