...
We use the notion known as \emph{homomorphisms} to describe how an instance satisfies a conjunctive query. In the literature, it is common to define homomorphisms as unrestricted substitutions of variables and nulls to terms. However, we will use a much more constrained notion of homomorphisms in this report.
...
% TODO: Motivate why we want to compute Datalog rewritings by talking about data complexity, cite the result showing the existence and finally formally define.
...
% TODO: talk about Atomic Rewriting, which is a partial solution to the Datalog rewriting problem that works for atomic queries.
...
In \Cref{subquery-entailment-problem-section}, we define the precise structure of \emph{local instances} and discuss how we could efficiently enumerate instances that entail a subgoal. In \Cref{a-rewriting-algorithm}, we briefly discuss how to integrate subgoals and then present a rewriting algorithm. Finally, in \Cref{secomputation-optimisations}, we address crucial optimisations in the subquery entailment computation.
...
\begin{example}
  ...
  \begin{align*}
    ...
    &\wedge R(z_3, z_4) \wedge R(z_3, z_5) \wedge T(z_2, z_6, z_5)
  \end{align*}
  as in \Cref{query-connectedness-example}. Then $\partial_Q \{z_4\} = \{z_3\}$, $\partial_Q \{z_2\} = \{z_1, z_3, z_5, z_6\}$ and $\partial_Q \{z_5, z_6\} = \{z_2, z_3\}$. The first example is illustrated in \Cref{query-boundary-example}.
  \qed
\end{example}
...
It is clear that, under the identification of the subgoal atom $\mathrm{Subgoal}_{Q,C}(\vec{t})$ with the subquery $\Subq{Q}{\left\{ \overrightarrow{\partial_Q C} \mapsto \vec{t} \right\}}{C}$ of $Q$, the rule $\mathrm{SubgoalRule}(\mathcal{I})$ is sound if and only if $\mathcal{I}$ is an \textsc{yes} instance of \Cref{subquery-entailment-problem}.

\section{A Rewriting Algorithm}
\label{a-rewriting-algorithm}

To produce a Datalog rewriting of a pair $(\Sigma, Q)$, it now remains to enumerate subquery entailment instances satisfying \Cref{subquery-entailment-problem}.

We can solve \Cref{subquery-entailment-problem} with only a minor modification to \Cref{basic-query-answering-procedure}, with the basic idea being that we descend the $\SCTree{I}{\Sigma}$ to search for a successful commit point. The only difference is that when we fire an existential rule, we must not drop local names in $\range{\sigma_I} \cup \range{\iota}$, since otherwise the partially substituted subquery becomes immediately unsatisfiable in the chased subtree. The corresponding pseudocode can be found in \Cref{naive-subquery-entailment-computation}. (TODO: this is not the actual subquery entailment algorithm)

% TODO: revise this!
\begin{algorithm}
\caption{Naive Subquery Entailment Computation}
\label{naive-subquery-entailment-computation}
\begin{algorithmic}[1]
\State // decide if there exists a $\Consts(\Sigma)$-free query homomorphism
\State // $\sigma: Q \rightarrow \SCTree{\mathcal{D}}{\Sigma}$ for connected BCQ $Q$
\Procedure{EntailsConnectedBCQ}{$\mathcal{D}$, $\Sigma$, connected BCQ $Q$}
...
% TODO: We would love to write about the "short-circuiting" in the middle of DFS and re-normalisation of local instances to {0, ..., K-1}.
