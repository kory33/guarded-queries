...
\label{local-realization-of-subquery-representation}
  ...
  \[\begin{array}{rcl}
    ...
    \sigma_I &=& \{ z_4 \mapsto 3 \} \\
    \iota &=& \{ c_1 \mapsto 2, c_2 \mapsto 3 \}
  \end{array}\]
  where $r_1 \in \Consts(\Sigma)$, then $$
  \LRealz{Q}{\sigma_{\Consts(\Sigma)}, \sigma_I, \iota} = \exists z_1, z_2. S(2, z_1, 3, z_2) \wedge T(z_1, 3, r_1).
  $$
\end{example}
...
We can solve \Cref{subquery-entailment-problem} with only a minor modification to \Cref{basic-query-answering-procedure}, with the idea being that we descend the $\SCTree{I}{\Sigma}$ to search for a successful commit point. The only difference is that when we fire an existential rule, we must not drop local names in $\range{\sigma_I} \cup \range{\iota}$, otherwise, the subquery becomes immediately unsatisfiable in the chased subtree.
