...
In the settings of (existential) TGD rules, objects known as \emph{chases} represent \emph{repaired} database instances. Chases are constructed similarly to Datalog saturations by adding tuples produced by rules, except we need to replace existential variables with \emph{nulls}, which represent fresh constants \cite{fagin_kolaitis_miller_popa_2005}.
...
However, in the presence of recursive rules such as $R(x, y) \rightarrow \exists z. R(y, z)$, the chase procedure must be continued indefinitely, leaving us with an infinite chase. Moreover, due to how one constructs a chase, it lacks a structure with which we can reason about query entailments.
...
\label{chase-proof-completeness}
  ...
  \begin{proof}[Proof (sketch)] $ $\par
    ($\Longrightarrow$, "soundness" of chase proofs): Suppose that $(T_1, \mu_1), \ldots, (T_n, \mu_n)$ is a tree-like chase proof over $\Sigma$ from $\mathcal{D}$. Take any (Herbrand) structure $\mathcal{M}$ and suppose $\mathcal{M} \models \mathcal{D} \wedge \Sigma$. Write $I_i = \bigcup_{v \in T_i} \mu_i(v)$ for the $i$-th chased instance.
    ...
    ($\Longleftarrow$, "completeness" of chase proofs): This direction is proven in \cite[Proposition 2.6.9]{kappelmann_2019}. The cited proposition shows that an ordinary chase be decomposed into a tree-like structure, and the decomposition is performed exactly as in the definition of \ref{tree-like-chase-proof}.
  \end{proof}
\end{proposition}
...
\begin{proposition}
  Let $\Sigma$ be a set of single-headed GTGDs and $\mathcal{D}$ an instance. Let $K$ be the maximum arity of predicates that appear in $\mathcal{D} \cup \Sigma$. Then every instance at a non-root node of \emph{$\SCTree{\mathcal{D}}{\Sigma}$} is $(K, \Sigma)$-small. In particular, if $\mathcal{D}$ is $(K, \Sigma)$-small, all instances in $\SCTree{\mathcal{D}}{\Sigma}$ are $(K, \Sigma)$-small.
  \begin{proof}
