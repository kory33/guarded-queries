...
\label{bcq-base-connected-query-decomposition}
  Let $Q = \exists \vec{z}. \bigwedge_{j \in J} A_j$ be a Boolean conjunctive query, $\mathcal{D}$ an instance and $\Sigma$ a finite set of GTGDs. Then $\mathcal{D} \cup \Sigma \models Q$ if and only if there exists a partial map $\sigma_\mathrm{partial}: \vec{z} \rightharpoonup \Terms(\mathcal{D})$ such that
  \begin{itemize}
...
\label{base-connected-query-decomposition}
  Let $Q = \exists \vec{z}. \bigwedge_{j \in J} A_j$ be a Boolean conjunctive query, $\mathcal{D}$ an instance and $\Sigma$ a finite set of GTGDs.
...
In \Cref{characterising-query-entailment-under-gtgds}, we devised \Cref{basic-query-answering-procedure} for producing all answers to a conjunctive query over GTGDs.

However, because of high data complexity, the algorithm is impractical for query-answering purposes. We have to explore a part of $\SCTree{\mathcal{D}}{\Sigma}$ for every single substitution $\alpha: FV(Q) \rightarrow \Consts(\mathcal{D})$, so the running time is inevitably exponential to the number of constants in the input base instance $\mathcal{D}$.

Instead, we aim to use \Cref{basic-query-answering-procedure} as a stepping stone to come up with a Datalog rewriting that works for arbitrary input $\mathcal{D}$.

The first important observation is that if a subquery is satisfied within a proper subtree $\SCTree{\mathcal{D}}{\Sigma} \downarrow c$ of $\SCTree{\mathcal{D}}{\Sigma}$ rooted at a child $c$ of the root node, then as long as enough facts are inherited to $c$, the subquery remains entailed in the subtree $\SCTree{\mathcal{D}}{\Sigma} \downarrow c$ even when uninherited facts are removed from the base instance $\mathcal{D}$. In other words, the subquery entailment only depends on the set of facts inherited from the base instance.

% TODO: add a diagram if possible.

If an existential rule $\forall \vec{x}. \beta \rightarrow \exists \vec{y}. H(\vec{x}, \vec{y})$ is fired with a substitution $\sigma: \vec{x} \rightarrow \Terms(\mathcal{D})$ to grow a child node $c$, the only facts inherited to $c$ are the ones whose terms all lie in $\sigma(H(\vec{x}, \vec{y}))$.

% TODO: write about the general strategy for producing a rewriting
%       where one prepares "subgoal atoms" for each connected variable set of the query
%       and then come up with "sufficiently large" base data to entail the subquery,
%       finally adding in all "subquery integration rules".

\section{What Does It Take To Entail a Subquery?}
