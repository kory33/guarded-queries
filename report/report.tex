...
Recall that a subgoal atom $\mathrm{Subgoal}_{Q, C}(\vec{t})$ indicates that the set $C$ of bound variables are witnessed existentially with a partial substitution $\left\{ \overrightarrow{\partial_Q C} \mapsto \vec{t} \right\}$ to the boundary of $C$. By \Cref{base-connected-query-decomposition}, if we fix the set $BVars \supseteq FV(Q)$ of variables to be witnessed at the base, we can add the following rule.
