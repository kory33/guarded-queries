...
% TODO: define TGDs, Datalog rules, full TGDs, guarded TGDs and single-headed GTGDs (whose existential heads are single-headed).
% TODO: mention how we can turn arbitrary set of GTGDs into single-headed GTGDs by introduce auxiliary, or intensional, predicates
...
\begin{figure}[t]
  ...
  \caption{A shortcutting chase tree over an instance $\{ A(c_4, c_1) \}$ with rules in \Cref{tree-like-chase-proof-example}. The facts highlighted in red are the ones obtained by Datalog-saturating the inherited bag with an atomic rewriting, and dotted boxes indicate substituted bodies used to fire an existential rule. Notice how the subtree rooted at $v_1$ can be obtained as a shortcutting chase of the bag at $v_1$, as remarked in \Cref{coinductiveness-of-sctree}.}
...
\label{recursive-connected-bcq-entailment}
  ...
  \begin{proof}
    ...
    By \Cref{coinductiveness-of-sctree}, $T_{\Vars(Q)} = \SCTree{\mu(r)}{\Sigma}$. Since terms in $\sigma(\Vars(Q))$ only appear in $T_{\Vars(Q)}$, $\sigma$ is a $\Consts(\Sigma)$-free query homomorphism $\sigma: Q \rightarrow \SCTree{\mu(r)}{\Sigma}$. Let $B = \{ v \in \Vars(Q) \mid \sigma(v) \in \Terms(\mu(r)) \}$ be a nonempty set of variables mapped to terms in $r$ and define $\sigma_\mathrm{commit} = \sigma \upharpoonright B$, then $\sigma_\mathrm{commit}$ is a commit map at $r$.
...
In \Cref{local-instances-and-the-subquery-entailment-problem}, we define the precise structure of \emph{local instances} and discuss how we could efficiently enumerate instances that entail a subgoal. In \Cref{a-rewriting-algorithm}, we briefly discuss how to integrate subgoals and then present a rewriting algorithm. Finally, in \Cref{secomputation-optimizations}, we address crucial optimisations in the subquery entailment computation.
...
\label{informal-subquery-entailment-problem}
  Suppose $I$ is a certain $(K, \Sigma)$-small structure, $C$ a $Q$-connected set of bound variables and $Q_{\mathrm{sub}, C}$ a subquery of $Q$ with existentials $C$. Is $I$ large enough to entail $Q_{\mathrm{sub}, C}$?
\end{problem}
...
\begin{figure}[ht]
  ...
  \caption{(Left) The shortcutting chase from \Cref{sctree-simple-example-diagram}. (Right) relabelling of the shortcutting chase tree with local names. The same local name in sibling nodes (e.g. $2$ in $v'_2$ and $v'_3$) may represent different terms.}
...
\begin{definition}
  For a conjunctive query $Q = \exists \vec{z}. \bigwedge_{j \in J} A_j$ and a $Q$-connected subset $C$ of $\vec{z}$, the \emph{$Q$-neighbourhood} (written $\partial_Q C$) of $C$ is the set of (either free or bound) variables in $Q$ defined by $$
    \partial_Q C = \left\{
...
\begin{example}
  ...
  \begin{align*}
    ...
    &\wedge R(z_3, z_4) \wedge R(z_3, z_5) \wedge T(z_2, z_6, z_5).
  \end{align*}
  as in \Cref{query-connectedness-example}. Then $\partial_Q \{z_4\} = \{z_3\}$ (\Cref{query-neighbourhood-example}), $\partial_Q \{z_2\} = \{z_1, z_3, z_5, z_6\}$ and $\partial_Q \{z_5, z_6\} = \{z_2, z_3\}$.
  \qed
\end{example}

\begin{figure}[h]
  \centering
  \includesvg{./diagrams/query-neighbourhood-example.drawio.svg}
  \caption{For the query from \Cref{query-connectedness-example}, the $Q$-neighbourhood of $\{z_4\}$ (red) is $\{z_3\}$ (blue).}
  \label{query-neighbourhood-example}
\end{figure}

A variable in $\partial_Q C$ may be mapped either
\begin{itemize}
  \item to a constant in $\Consts(\Sigma)$ at the beginning, or
  \item to a local name while trying to find a successful commit point.
\end{itemize}
Putting these concepts together, we obtain the following notion of a subquery.

\begin{definition}
  Let $Q$ be a conjunctive query, $C$ be a $Q$-connected subset of bound variables in $Q$ and $I$ a $(K, \Sigma)$-local instance. A \emph{representation of a subquery of $Q$ induced by $C$ local to $I$} is a pair $(\sigma_{\Consts(\Sigma)}, \sigma_I)$ of partial mappings
  \[\begin{array}{rccl}
    \sigma_{\Consts(\Sigma)}: & \partial_Q C &\rightharpoonup& \Consts(\Sigma) \\
    \sigma_I: & \partial_Q C &\rightharpoonup& \LNames{I}
  \end{array}\]
  whose domains partition $\partial_Q C$.
\end{definition}

Finally, we must always embed non-$\Consts(\Sigma)$ query constants in the subquery into the local instance; otherwise, we cannot hope for a query entailment. For example, we cannot hope to entail a query $\exists y. T(x, y, c)$ (where $c \not\in \Consts(\Sigma)$) from a local instance $I$ without having a local name in $\LNames{I}$ representing the constant $c$.

Combining notions defined so far, we are ready to formalise \Cref{informal-subquery-entailment-problem}.

\begin{definition}
  Let $Q = \exists \vec{z}. \bigwedge_{j \in J} A_j$ be a conjunctive query. A \emph{$Q$-subquery entailment problem instance} is a 5-tuple $(C, I, \sigma_{\Consts(\Sigma)}, \sigma_I, \iota_C)$ where
  \begin{itemize}
    \item $C$ is a $Q$-connected set of variables bound in $Q$
    \item $I$ is a $(K, \Sigma)$-local instance
    \item $(\sigma_{\Consts(\Sigma)}, \sigma_I)$ is a representation of a subquery of $Q$ induced by $C$ local to $I$
    \item $\iota_C$ is an embedding of query constants to the set $\LNames{I}$. More formally, let $$
      S = \left\{
      ...
      \right\},
    $$
    be the set of constants in the subquery induced by $C$, then $\iota_C: S \hookrightarrow\nobreak \LNames{I}$.
  \end{itemize}

  Given a $Q$-subquery entailment problem instance $\mathcal{I} = (C, I, \sigma_{\Consts(\Sigma)}, \sigma_I, \iota_C)$, its \emph{local realization}, written $\LRealz{Q}{\mathcal{I}}$, is the Boolean conjunctive query $$
  \LRealz{Q}{\mathcal{I}} = \exists \vec{C}. \bigwedge_{j \in J_C} ((\sigma_{\Consts(\Sigma)} \cup \sigma_I) \circ \iota_C)(A_j)
  $$
  where $J_C = \{ j \in J \mid \Vars(A_j) \cap C \neq \empty \}$.
  \qed
\end{definition}

\begin{problem}[Subquery Entailment Problem]
\label{subquery-entailment-problem}
  Given a $Q$-subquery entailment problem instance $\mathcal{I} = (C, I, \sigma_{\Consts(\Sigma)}, \sigma_I, \iota_C)$, does $\mathcal{I} \cup \Sigma \models \LRealz{Q}{\mathcal{I}}$ hold?
\end{problem}
...
% TODO: We would love to write about the "short-circuiting" in the middle of DFS and re-normalization of local instances to {0, ..., K-1}.
