...
% Load `.bib` file
% TODO: we should get rid of "et al." 
...
We now discuss how the structure of the query constrains the structure of query homomorphisms into the shortcutting chase tree.

% TODO: briefly mention why we are going into this stuff.
...
\begin{definition}
  Let $Q = \exists \vec{z}. \bigwedge_{j \in J} A_j$ be a conjunctive query, $\SCTree{\mathcal{D}}{\Sigma}$ a shortcutting chase tree over an instance (which may contain nulls) and \newline $\sigma_\mathrm{partial}: \Vars(Q) \rightharpoonup \Terms(\mathcal{D})$ a partial map from query variables to terms in $\mathcal{D}$. Let $\mathrm{BVars} = \dom{\sigma_\mathrm{partial}}$.
...
\label{base-connected-query-decomposition}
  Let $Q = \exists \vec{z}. \bigwedge_{j \in J} A_j$ be a conjunctive query, $\mathcal{D}$ an instance and $\Sigma$ a finite set of GTGDs. Then, for a substitution\newline $\alpha: FV(Q) \rightarrow \Terms(\mathcal{D})$, $\mathcal{D} \cup \Sigma \models \alpha(Q)$ if and only if there exists an extension $\sigma_\mathrm{base}: \Vars(Q) \rightharpoonup \Consts(\mathcal{D})$ of $\alpha$ such that
  \begin{itemize}
  ...
  \begin{proof}
    ...
    ($\Longleftarrow$): Suppose that $\sigma_\mathrm{base}$ satisfies the two conditions, and let $C_1, \ldots, C_n$ be $Q$-connected components of $(\vec{z} \setminus\nobreak \mathrm{\dom{\sigma_\mathrm{base}}})$.
    
    For each $1 \leq i \leq n$, we can take a query homomorphism $\sigma_{C_i}: \Subq{Q}{\sigma_\mathrm{base}}{C_i} \rightarrow \SCTree{\mathcal{D}}{\Sigma}$ by \Cref{sctree-soundness-completeness}. Since all of $\sigma_\mathrm{base}, \sigma_{C_1}, \ldots, \sigma_{C_n}$ have disjoint domains, $\sigma = \sigma_\mathrm{base} \cup \sigma_{C_1} \cup \ldots \cup \sigma_{C_n}$ is a function $\sigma: \Vars(Q) \rightarrow \Terms(\SCTree{\mathcal{D}}{\Sigma})$.
    
    Since every atom $A_j$ in $Q$ appears either in $\Baseq{Q}{\sigma_\mathrm{base}}$ or in precisely one $\Subq{Q}{\sigma_\mathrm{base}}{C_i}$, $\sigma$ is a query homomorphism $\sigma: Q \rightarrow \SCTree{\mathcal{D}}{\Sigma}$.
    
    As $\sigma \supseteq \sigma_\mathrm{base} \supseteq \alpha$, $(\sigma \upharpoonright \vec{z})$ is a query homomorphism $(\sigma \upharpoonright \vec{z}): \alpha(Q) \rightarrow\nobreak \SCTree{\mathcal{D}}{\Sigma}$. Therefore $\mathcal{D} \cup \Sigma \models \alpha(Q)$.
  \end{proof}
\end{lemma}

% TODO: put a helpful diagram

In order to produce answers to the query using \Cref{base-connected-query-decomposition}, we need to decide whether or not $\mathcal{D} \cup \Sigma \models \Subq{Q}{\sigma_\mathrm{base}}{C}$ holds. It turns out that we can exploit the connectedness of $\Subq{Q}{\sigma_\mathrm{base}}{C}$ and the coinductive structure of $\SCTree{\mathcal{D}}{\Sigma}$ to check the query entailment recursively.
...
% TODO: talk about "replacing \mathcal{D} with the head of the tentacle?" first as an "optimization" of the above algorithm, and state that this is not just an optimization 
