...
\Cref{coexistentialness} asserts that a homomorphism $\sigma: Q \rightarrow \SCTree{\mathcal{D}}{\Sigma}$ forces connected sets of $\textrm{NullVars}$ to appear in the same tentacle. In light of this, we call a $Q$-connected component of $\NullVars{\sigma}$ a \emph{set of variables co-existential according to $\sigma$}.
...
We are now ready to state two main results of this section, which we will use in \Cref{deriving-a-rewriting} to derive the Datalog rewriting.
