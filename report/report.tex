...
% TODO: we do not want spaces after operator names
...
\begin{itemize}
  ...
  \item For each \emph{local instance} $I$ (to be defined in \Cref{subquery-entailment-problem-section}), which represents a $(K, \Sigma)$-small structure in the base instance, decide if $I$ contains enough facts to entail a subquery induced by $C$. If so, add a rule roughly of the form $I \rightarrow \mathrm{Subgoal}_C$.
...
Notice that, as shown in \Cref{sctree-simple-example-local-names}, we can translate a $\SCTree{\mathcal{D}}{\Sigma}$ into a tree-like structure that uses local names. By abuse of notation, we write $\SCTree{I}{\Sigma}$ for the tree-like structure obtained this way.
...
Later in \Cref{secomputation-optimisations}, we discuss optimisations of \Cref{naive-subquery-entailment-enumeration}. Nevertheless, for now, we move on to the other components of the rewriting algorithm to see how outputs from \Cref{naive-subquery-entailment-enumeration} can be assembled into a Datalog rewriting.
...
\Cref{algorithm:recursive-subquery-entailment-enumeration} is another implementation of \nobreak\textsc{EnumerateSubqueryEntailments} based on \Cref{definition:subquery-entailment-problem-splitting}. The algorithm is equivalent to \Cref{naive-subquery-entailment-enumeration}, but since \textsc{IsSubqueryEntailment} is a Boolean function over $(\Sigma, Q)$-subquery entailment problem instances, we can memoise answers of \textsc{IsSubqueryEntailment} and avoid recomputations.
...
Recall that we use $2K$ local names as the set of $K$ names is insufficient to maintain a tree-like structure of local instances (\Cref{sctree-simple-example-equivalence-classes}).
...
For comparison, with only (DP) and (DP + normalisation) optimisations applied, rewriting the same input as in \Cref{figure:example-command-line-interaction} takes about 7.3 seconds and 160 seconds respectively, demonstrating the effectiveness of optimisations in \Cref{secomputation-optimisations}.
