...
\title{Rewriting A Conjunctive Query Under Guarded TGDs}
...
% FIXME: This is false; we need to first guess rule constant guesses
\begin{theorem}
\label{recursive-connected-bcq-entailment}
  Let $Q = \exists \vec{z}. \bigwedge_{j \in J} A_j$ be a connected Boolean conjunctive query, $\mathcal{D}$ an instance and $\Sigma$ a finite set of single-headed GTGDs. Then $\mathcal{D} \cup \Sigma \models Q$ if and only if there exists a successful commit point for $Q$ in $\SCTree{\mathcal{D}}{\Sigma}$.
  \begin{proof}
...
\Procedure{IsSuccessfulBranchingPoint}{$\mathcal{D}$, $\Sigma$, connected BCQ $Q$}
  \ForAll{nonempty partial map $\sigma_\mathrm{commit}: \Vars(Q) \rightharpoonup \Terms(\mathcal{D})$}
...
\label{implicit-equality-coding-of-local-names}
  Consider the following (TODO: translate \Cref{sctree-example-diagram} together with a suitable query into a local instance).
\end{example}
