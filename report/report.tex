...
\begin{figure}[t]
  ...
  \caption{A tree-like chase proof with rules in \Cref{tree-like-chase-proof-example}}
...
% TODO: It may be better to call this quantity "width", but it is slightly different from treewidth since we exclude rule constants. I could not come up with a fitting word here.
...
\end{itemize}
We can immediately achieve the former objective by computing the Datalog saturation of the bag at $n$ using $\ARew{\Sigma}$. This observation motivates the following definition.
...
\begin{example}
  Consider the rules in \Cref{tree-like-chase-proof-example}. According to an implementation in \cite{Benedikt_Rewriting_the_Infinite}, the set $\Sigma'$ containing the following two rules is an atomic rewriting.
  \[\begin{array}{ccrcl}
    \tau'_1 & = & \forall x_1, x_2.\ B(x_1, x_2) & \rightarrow & C(x_1, c_1) \wedge A(x_2, c_1) \\
    \tau'_2 & = & \forall x_1, x_2.\ A(x_1, x_2) & \rightarrow & C(x_1, c_1)
  \end{array}\]

  A first few layers of $\SCTree{\{ A(c_4, c_1) \}}{\Sigma}$ is illustrated in \Cref{sctree-example-diagram}. The chase tree is continued indefinitely by chasing with existential rules and Datalog saturating each layer with $\Sigma'$.
  \qed
\end{example}

\begin{figure}[t]
  \centering
  \includesvg{./diagrams/sctree-example.drawio.svg}
  \caption{A shortcutting chase tree over an instance $\{ A(c_4, c_1) \}$ with rules in \Cref{tree-like-chase-proof-example}. Facts highlighted in red are the ones obtained by Datalog saturating the inherited bag with an atomic rewriting, and dotted boxes indicate substituted bodies used to fire an existential rule. Notice how the subtree under $v_1$ can be obtained by shortcutting chasing the bag at $v_1$, as remarked in \Cref{coinductiveness-of-sctree}.}
  \label{sctree-example-diagram}
\end{figure}
...
% FIXME: This lemma does not hold when a homomorphism maps a variable to a rule constant! A rule constant may disappear and then appear deep in the sctree since it can be created out of nowhere provided there are enough facts (maybe we should remark this?).
...
\begin{figure}[ht]
  ...
  \caption{Decomposition of a query in \Cref{bcq-decomposition-example}.}
...
\label{bcq-decomposition-example}
  ...
  \begin{align*}
    ...
    &\wedge R(z_3, z_4) \wedge R(z_3, z_5) \wedge T(z_2, z_6, z_5).
  \end{align*}
  as in \Cref{query-connectedness-example}. Suppose that $\sigma_\mathrm{partial}$ is given by \[\begin{array}{cccc}
    \sigma_\mathrm{partial}: &\Vars(Q) &\rightharpoonup &\{c_1, c_2, c_3\} \\
    &z_2 &\mapsto &c_1 \\
    &z_3 &\mapsto &c_3
  \end{array}\]
  then $\Baseq{Q}{\sigma_\mathrm{partial}} = R(c_1, c_3)$. The split connected components are $C_1 = \{z_1\}$, $C_2 = \{z_4\}$ and $C_3 = \{z_5, z_6\}$, and their corresponding subqueries are
  \begin{align*}
    \Subq{Q}{\sigma_\mathrm{partial}}{C_1} &= \exists z_1.\ S(z_1, c_1) \wedge S(z_1, c_3) \\
    \Subq{Q}{\sigma_\mathrm{partial}}{C_2} &= \exists z_4.\ R(c_3, z_4) \\
    \Subq{Q}{\sigma_\mathrm{partial}}{C_3} &= \exists z_5, z_6.\ R(c_3, z_5) \wedge T(c_1, z_6, z_5)
  \end{align*}
  The decomposition after applying $\sigma_\mathrm{partial}$ is illustrated in \Cref{bcq-decomposition-example-figure}.
  \qed
\end{example}
...
\begin{itemize}
  \item there are only finitely many such classes by \Cref{each-bag-in-sctree-is-k-sigma-small}, and
...
In \Cref{local-instances-and-the-subquery-entailment-problem}, we define the precise structure of \emph{local instances} and discuss how to enumerate instances that entail a subgoal. In \Cref{a-rewriting-algorithm}, we briefly discuss how to integrate subgoals and then present a rewriting algorithm.

\section{Local Instances and the Subquery Entailment Problem}
\label{local-instances-and-the-subquery-entailment-problem}

To proceed with the strategy, we need to define a data structure representing an input to the following question: "Is this $(K, \Sigma)$-small structure sufficiently large to entail a subquery of $Q$ induced by a $Q$-connected set $C$ of bound variables?"

Moreover, we would like the multiple instances of structures to interconnect each other so that they can describe a local structure of $\SCTree{\mathcal{D}}{\Sigma}$. We illustrate this problem and a solution we will adopt with the following example.

\begin{example}
\label{implicit-equality-coding-of-local-names}
  Consider the following (TODO: define a rule set and pick a base instance, and then draw a $\SCTree{\mathcal{D}}{\Sigma}$).
\end{example}
