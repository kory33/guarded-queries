...
\begin{definition}
  ...
  Given such a triple, its \emph{local realization at $I$}, written $\LRealz{Q}{\sigma_{\Consts(\Sigma)}, \sigma_I, \iota}$, is the connected Boolean conjunctive query $$
...
\begin{example}
\label{local-realization-of-subquery-representation}
  Let
  $$
...
For example, suppose that the subquery in \Cref{local-realization-of-subquery-representation} is entailed by the local instance $I$ given in the example. We would then like to add a rule $$
...
It is clear that, under the identification of the subgoal atom $\mathrm{Subgoal}_{Q,C}(\vec{t})$ with the subquery $\Subq{Q}{C}{\left\{ \overrightarrow{\partial_Q C} \mapsto \vec{t} \right\}}$ of $Q$, the rule $\mathrm{SubgoalRule}(\mathcal{I})$ is sound if and only if $\mathcal{I}$ is an \textsc{yes} instance of \Cref{subquery-entailment-problem}.
